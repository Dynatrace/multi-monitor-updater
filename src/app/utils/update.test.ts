...
import { BulkConfig, ConfigParam, InitialBulkConfig, OutageHandlingParam } from './models';
...
describe('testing initiation of a update', () => {
  test('getUpdateDto should return expected object', () => {
    expect(getUpdateDto(existing, initialBulkConfig, changedBulkConfig, 'same-type')).toMatchObject(expected);
  });
  test('only outage should changed', () => {
    ...
    expect(dto.anomalyDetection?.outageHandling.globalOutagePolicy?.consecutiveRuns).toEqual(
      changedOutageParam.globalOutagePolicy.consecutiveRuns,
    );
  });
