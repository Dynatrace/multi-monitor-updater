...
export const MonitorConfigPreview = ({ monitorId }: MonitorConfigProps) => {
  ...
  if (error) {
    return (
      <Text textStyle="base-emphasized">Up&apos;s something went wrong!</Text>
    );
  }
  ...
  return configAsString !== "" ? (
    ...
    <Text textStyle="base-emphasized">
      No monitor selected. Click one of the names in the list to display the
      configuration in JSON format here.
    </Text>
  );
};
