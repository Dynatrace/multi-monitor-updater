...
import {
  getUpdateDto,
  getUpdateDtoForCurrentSelection,
} from "../../utils/update";
...
const updateParamStateReducer: (
  state: BulkConfig,
  action: ConfigParamChangeAction
) => BulkConfig = (state, action) => {
  switch (action.type) {
    ...
    case ConfigParam.OUTAGE_HANDLING:
      return {
        ...
        outageHandling: {
          ...
          globalOutagePolicy: {
            consecutiveRuns:
              action.updatedValue.outageHandling?.globalOutagePolicy
                .consecutiveRuns ?? "",
          },
        },
      };
...
export const BulkUpdateModal = ({
  ...
  const [selectedParam, setSelectedParam] = useState<ConfigParam[] | null>([
    ConfigParam.OUTAGE_HANDLING,
  ]);
  const [saveCurrentOnly, setSaveCurrentOnly] = useState<boolean>(false);
  const [updatedBulkConfig, dispatchParamChange] = useReducer(
    updateParamStateReducer,
    {}
  );

  const httpMonitorCount = selectedIds.filter((id) =>
    id.startsWith("HTTP_CHECK")
  ).length;

  const currentMode =
    httpMonitorCount === selectedIds.length || httpMonitorCount === 0
      ? "same-type"
      : "mixed";

  const availableConfigParameters =
    currentMode === "same-type" ? allConfigParameters : commonConfigParameters;
  ...
  const data = results
    ...
    .map((result) => result.data as SyntheticMonitor);
  // const progress = selectedIds.length > 0 ? data.length / results.length : 1;

  const initialBulkConfig = useMemo<InitialBulkConfig | undefined>(() => {
    return data.length > 0 && isSuccess
      ? getInitialBulkConfig(data)
      : undefined;
  }, [data, isSuccess]);
  ...
  return initialBulkConfig ? (
    <Fragment>
      ...
      <Switch
        name="update-scope"
        value={saveCurrentOnly}
        onChange={setSaveCurrentOnly}
      >
      ...
      {updateStatus !== "idle" ? (
        <ProgressBar
          value={progress}
          max={1}
          variant={progressVariant(updateStatus)}
        >
      ...
      <Flex flexDirection="row" justifyContent="flex-start">
        {updateStatus === "success" || updateStatus === "error" ? (
          ...
          <>
            <Button
              color="primary"
              disabled={!isSuccess || updateStatus === "loading"}
